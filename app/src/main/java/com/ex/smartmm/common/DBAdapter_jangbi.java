...
public class DBAdapter_jangbi {
	...
	/** 정비관리 데이터 조회 **/
	public Cursor select_jeongbi(String dogongcode, String jangbimyeong, String bonbucode, String jisacode){
		Log.d(TAG,TAG+" select_jeongbi Method call!");
		Log.d(TAG, "select_jeongbi()~!");
		Log.d(TAG, "select_jeongbi() - dogongcode : "+dogongcode);
		Log.d(TAG, "select_jeongbi() - jangbimyeong : "+jangbimyeong);
		Log.d(TAG, "select_jeongbi() - bonbucode : "+bonbucode);
		Log.d(TAG, "select_jeongbi() - jisacode : "+jisacode);
		//insertJeongbiYong(jisacode);
		getSqlite().beginTransaction();
		Cursor cursor = null;
		Cursor cursor2 = null;
		
		try {
			cursor2 = getSqlite().rawQuery("select * from T_EXMB_EQPM01M1 where MGMT_DPTCD='N02677'", null);
			Log.d(TAG,TAG+" select_jeongbi Method call " +cursor2.getCount());

			for (int ii=0; ii<cursor2.getCount(); ii++){
				cursor2.moveToPosition(ii);

				Log.d(TAG,TAG+" select_jeongbi Method call " +cursor2.getString(0)+":"+cursor2.getString(1)
						+":"+cursor2.getString(2)
						+":"+cursor2.getString(3)
						+":"+cursor2.getString(4)
						+":"+cursor2.getString(5)
						+":"+cursor2.getString(6)
						+":"+cursor2.getString(42)
						+":"+cursor2.getString(43)
						+":"+cursor2.getString(44)
				);
			}
			...
			sql.append(" 	'' AS BUSEOMYEONG, 																	\n");
			sql.append(" 	'"+jisacode+"' AS BUSEOCODE, 														\n");
			sql.append(" 	'"+bonbucode+"' AS GIGYANMYEONG, 													\n");
			//sql.append(" 	'-' AS CODE, 																		\n");
			//sql.append(" 	'-' AS CODEGUBUN, 																	\n");
			sql.append(" 	'-' AS JSTIME, 																		\n");
			sql.append(" 	'!' AS DOGONGSORT 																	\n");
			//sql.append(" 	'Y' AS USE_YN, 																		\n");
			sql.append(" FROM T_EXMB_EQPM01M1 																	\n");
			sql.append(" WHERE EQPM_CD='50000001' 																\n");
			sql.append(" UNION ALL 																				\n");
			sql.append(" SELECT																					\n");
			sql.append(" 		CODEMYEONG, JANGBICODE, JANGBIMYEONG, DOGONGCODE, CHAJONGCODE,					\n");
			sql.append(" 		DEUNGROKNO, GYUGYEOK, PIBUSEOMYEONG, BUSEOMYEONG, BUSEOCODE, GIGYANMYEONG,					\n");
			sql.append(" 	JSTIME, DOGONGSORT																	\n");
			sql.append(" FROM (																					\n");
			sql.append(" 	SELECT 																				\n");
			sql.append(" 		C.GIGYANMYEONG																	\n");
			sql.append(" 		,C.BUSEOMYEONG														\n");
			sql.append(" 		,A.EQPM_CD AS JANGBICODE 														\n");
			sql.append(" 		,A.EQPM_NM AS JANGBIMYEONG														\n");
			sql.append(" 		,A.EQPM_RGST_CTNT AS DEUNGROKNO													\n");
			sql.append(" 		,G.CMMN_CD_NM CODEMYEONG														\n");
			sql.append(" 		,A.STNDS_NM AS GYUGYEOK															\n");
			sql.append(" 		,A.ACQS_DATES AS CHUIDEUKDATE													\n");
			sql.append(" 		,A.ACIT_CD AS GYEOIJEONGCODE													\n");
			sql.append(" 		,A.ASET_MTIT_CD AS JSPUMMOKCODE													\n");
			sql.append(" 		,A.FSTTM_ENRL_DPTNM AS PIBUSEOMYEONG											\n");
			sql.append(" 		,C.BUSEOCODE											\n");
			sql.append(" 		,A.EX_EQPM_IDNT_ID AS DOGONGCODE												\n");
			sql.append(" 		,A.EQPM_KNCR_CD AS CHAJONGCODE													\n");
			sql.append(" 		,A.DSUSE_DATES AS BULYONGDATE 													\n");
			...
			sql.append(" 	LEFT OUTER JOIN V_CNKC_ASET_MTIT01C1 D												\n");
			sql.append(" 	on A.ASET_MTIT_CD = D.ASET_MTIT_CD													\n");
			sql.append(" 	and A.ACIT_CD = D.ACIT_CD															\n");
			sql.append(" 	LEFT OUTER JOIN T_EXMB_ERDY_SOP_RSLTS01L1 E											\n");
			sql.append(" 	ON A.EQPM_CD = E.EQPM_CD															\n");
			sql.append(" 	LEFT OUTER JOIN T_EXMB_EQPM_SPCC_ITSL01L1 F											\n");
			sql.append(" 	ON A.EQPM_CD = F.EQPM_CD															\n");
			sql.append(" 	LEFT OUTER JOIN T_CNKC_CMMN_CD01C1 G 												\n");
			sql.append(" 	ON  A.EQPM_KNCR_CD = G.CMMN_CD														\n");
			sql.append(" 	AND G.CMMN_GRP_CD = 'EQPM_KNCR_CD'													\n");
			sql.append(" 	LEFT OUTER JOIN T_CNKC_CMMN_CD01C1 H												\n");
			sql.append(" 	ON  A.KDOL_KIND_CD = H.CMMN_CD														\n");
			sql.append(" 	AND H.CMMN_GRP_CD = 'KDOL_KIND_CD',													\n");
			sql.append(" 	( SELECT																			\n");
			sql.append(" 		B.INTG_DPTCD AS GIGYANCODE														\n");
			sql.append(" 		,B.KOR_DPTNM AS GIGYANMYEONG													\n");
			sql.append(" 		,A.INTG_DPTCD AS BUSEOCODE														\n");
			sql.append(" 		,A.KOR_DPTNM AS BUSEOMYEONG														\n");
			sql.append(" 	  FROM V_CNKC_INTG_DPRT01C1 A, V_CNKC_INTG_DPRT01C1 B								\n");
			sql.append(" 	  WHERE A.PRAF_USE_YN = 'Y'															\n");
			sql.append(" 	  AND B.PRAF_USE_YN = 'Y'															\n");

//			sql.append(" 	  AND B.INTG_DPTCD = case A.IMDT_HGRN_DPTCD											\n");
//			sql.append(" 						 when 'N00001' then 'N00002'									\n");
//			sql.append(" 						 else ( case SUBSTR(A.HGRN_DPTCD, '7', '6')						\n");
//			sql.append(" 								when 'N00002' then 'N00002'								\n");
//			sql.append(" 								when 'N00119' then 'N00119'								\n");
//			sql.append(" 								when 'N00204' then SUBSTR(A.HGRN_DPTCD, '13', '6')		\n");
//			sql.append(" 								else A.INTG_DPTCD end								\n");
//			sql.append(" 						 ) end															\n");
			sql.append(" 	  AND B.INTG_DPTCD = '"+bonbucode+"'												\n");//본부코드
			sql.append(" 	  AND A.INTG_DPTCD = '"+jisacode+"'													\n");//지사코드
			sql.append(" 	group by GIGYANCODE, \tGIGYANMYEONG\t  , BUSEOCODE, BUSEOMYEONG						\n");
			sql.append(" 	) C																					\n");


			sql.append(" 	WHERE 1 = 1																			\n");
			sql.append(" 	AND A.MGMT_DPTCD = C.BUSEOCODE														\n");
			sql.append(" 	AND  (A.DSUSE_YN = 'N' OR A.DSUSE_YN IS NULL)										\n");
			sql.append(" 	ORDER BY C.BUSEOMYEONG, A.EQPM_CD													\n");
			sql.append(" )																						\n");
			...
			sql.append(" ORDER BY DOGONGSORT    											    				\n");
			Log.d(TAG, "select_jeongbi sql : "+sql.toString());
	...
	/** 정비관리 데이터 상세 조회 **/ 
	public Cursor select_jeongbi_detail(String bonbucode, String jisacode, String jangbicode) {
...
*/			
		...
		try {
			...
			if (cursor.getCount() > 0) {
				...
				if(lastUptDate.equals("") || lastUptDate.equals(null) ){
					...
					lastUptDate = "2020-04-01 09:00:00";
				}
	...
	public String setDataMngUpdate(String date){
		...
		try {
			...
			Log.e("setdataMngUpdate","setdataMngUpdate error ="+ e.toString());
		}finally{
			getSqlite().setTransactionSuccessful();
			getSqlite().endTransaction();
		}
	...
	/** 부서데이터 업데이트 **/
	public int update_bsinfo_new(JSONArray items){
		Log.d(TAG, "update_bsinfo_new()~!");
		Log.d(TAG, "update_bsinfo_new() - items size : "+items.length());
		getSqlite().beginTransaction();
		int rtnInt = 0;

		StringBuffer deleteSql1 = new StringBuffer();
		deleteSql1.append(" delete from bsinfo_new		\n");
		Log.d(TAG, "sql.toString() : "+deleteSql1.toString());
		getSqlite().execSQL(deleteSql1.toString());
...
//				Log.d("",TAG+" uptBsinfo_new() - items.get(" + i + ") : "+items.get(i));
				...
				bbcode = value.getString("bbcode");
				bbname = value.getString("bbname");
				bsname = value.getString("bsname");
				bscode = value.getString("bscode");
				if(value.has("sort")){
					sort = Common.nullCheck(value.getString("sort"));
				}else{
					sort = "";
				}


				insertSql1.append(" INSERT INTO bsinfo_new													\n");
				insertSql1.append("	 ( bbcode, bbname, bscode, bsname, useyn, sort)					\n");
				insertSql1.append(" VALUES															 			\n");
				insertSql1.append("	 ('"+bbcode+"','"+bbname+"','"+bscode+"','"+bsname+"','"+"','"+sort+"')		\n");

				Log.d(TAG, "sql.toString() : "+insertSql1.toString());
//				Log.d(TAG, "bsinfo_new	 ('"+bbcode+"','"+bbname+"','"+bscode+"','"+bsname+"','"+"','"+sort+"')		");
	...
	public int update_V_CNKC_INTG_DPRT01C1(JSONArray items){
		Log.d(TAG, "update_bsinfo_new()~!");
		Log.d(TAG, "update_bsinfo_new() - items size : "+items.length());
		getSqlite().beginTransaction();
		int rtnInt = 0;
		...
		StringBuffer deleteSql1 = new StringBuffer();
		deleteSql1.append(" delete from V_CNKC_INTG_DPRT01C1		\n");
		Log.d(TAG, "sql.toString() : "+deleteSql1.toString());
		getSqlite().execSQL(deleteSql1.toString());
...
//				Log.d("",TAG+" uptBsinfo_new() - items.get(" + i + ") : "+items.get(i));
				...
				insertSql1.append(" INSERT INTO V_CNKC_INTG_DPRT01C1													\n");
				insertSql1.append("	 ( intg_dptcd, kor_dptnm, PRAF_USE_YN)					\n");
				insertSql1.append(" VALUES																		\n");
				insertSql1.append("	 ('"+bscode+"','"+bsname+"','Y')		\n");
	...
	/** 본부명 조회 **/
	public String select_bbinfo_new(String bscode){
		Log.d(TAG, "update_bsinfo_new()~!");
		getSqlite().beginTransaction();
		Cursor cursor = null;
		String rtnBsName = "";
		...
		try {
			...
			selectSql.append(" SELECT BBNAME FROM BSINFO_NEW WHERE BBCODE='"+bscode+"' ");

			Log.d(TAG, "sql.toString() : "+selectSql.toString());
			cursor = getSqlite().rawQuery(selectSql.toString(), null);
			while (cursor.moveToNext()){
				rtnBsName = cursor.getString(0);
				break;
			}

			Log.d(TAG, "rtnBsName : "+rtnBsName);
		} catch (SQLException e) {
			Log.e("selectData()Error! : ", e.toString());
		}finally{
			getSqlite().setTransactionSuccessful();
	...
	/** 부서명 조회 **/
	public String select_bsinfo_new(String bscode){
		Log.d(TAG, "update_bsinfo_new()~!");
		getSqlite().beginTransaction();
		Cursor cursor = null;
		String rtnBsName = "";
		...
		try {
			selectSql.append(" SELECT BSNAME FROM BSINFO_NEW WHERE BSCODE='"+bscode+"' ");
	...
	public Cursor selectBms_BSINFO_BONBU() {
		...
		try {
			...
			sql.append(" ORDER BY CAST(sort as integer), bbcode                           \n");
...
*/			
			...
			sql.append(" group by bscode, bsname                            			\n");
